...
describe("Category Object", () => {
  it("can create a category object", done => {
...
describe("Store functions", () => {
  ...
  it("can toggle the selected category", () => {
    const store = CategoryStore.create({}, { api });
    expect(store.count).toBe(0);
    expect(store.isLoading).toBe(true);
  ...
  it("can return the tags for the categories which are selected", () => {
    const store = CategoryStore.create({}, { api });
    expect(store.count).toBe(0);
    expect(store.isLoading).toBe(true);
  ...
  it("clears all the selected categories", () => {
    const store = CategoryStore.create({}, { api });
    expect(store.count).toBe(0);
    expect(store.isLoading).toBe(true);
