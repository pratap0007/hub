  ...
  .actions((self) => ({
    load: flow(function* () {
      try {
        ...
        Array.from(kindData).forEach((kindName) => {
          self.kindStore.add({ name: kindName as string })
        })

        // let temp = [...Array.from(catalogData)]
        // temp.forEach((c) => {
        //   self.catalogStore.add({ name: c })
        // })

        Array.from(catalogData).forEach((catalogName) => {
          self.catalogStore.add({ name: catalogName as string })
        })
      } catch (err) {
